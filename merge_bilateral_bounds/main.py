...
def merge_files(file1, file2, out_dir):
    ...
    file1_arr[file2_arr == 255] = 255

    merged_label_img = sitk.GetImageFromArray(file1_arr)
    merged_label_img.CopyInformation(file1_img)

    out_path = os.path.join(out_dir, os.path.basename(file1.replace("_L_bounds.nii.gz", "_bounds_merged.nii.gz")))
    print("Merging: ")
    print(file1)
    print(file2, "------- >")
    print(out_path)
...
def get_pairs(i, out_dir):
    for fol, subs, files in os.walk(i, followlinks=True):
        for file in files:
            if file.endswith("_L_bounds.nii.gz"):
                f1 = os.path.join(fol, file)
                f2 = os.path.join(fol, file.replace("_L_bounds.nii.gz", "_R_bounds.nii.gz"))
