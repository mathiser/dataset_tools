...
label_map = {
    f"Brainstem_bounds.nii.gz": 1,
    f"SpinalCord_bounds": 2,
    f"Lips_bounds.nii.gz": 3,
    f"Esophagus_bounds.nii.gz": 4,
    f"PCM_Low_bounds.nii.gz": 5,
    f"PCM_Mid_bounds.nii.gz": 6,
    f"PCM_Up_bounds.nii.gz": 7,
    f"OralCavity_bounds.nii.gz": 8,
    f"Submandibular_bounds_merged.nii.gz": 9,
    f"Thyroid_bounds.nii.gz": 10,
    f"EyeFront_bounds_merged.nii.gz": 11,
    f"EyeBack_bounds_merged.nii.gz": 12
}
...
def merge_files(files: dict, out_dir):
    ...
    out_filename = os.path.basename(ref_path.rsplit("&", 1)[0]) + f"&significant_OAR_bounds_merged.nii.gz"
